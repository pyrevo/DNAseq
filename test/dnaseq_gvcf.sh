...
fasta=/media/Data4/ref/ucsc_hg19/hg19.fa
...
dbsnp=/home/massi/Downloads/dbsnp_138.hg19.vcf
known_Mills_indels=/home/massi/Downloads/Mills_and_1000G_gold_standard.indels.hg19.vcf
known_1000G_indels=/home/massi/Downloads/1000G_phase1.indels.hg19.vcf
...
# sentieon driver --interval $INTERVAL -r $fasta -t $nt -i deduped.bam -q recal_data.table --algo Haplotyper -d $dbsnp --emit_conf=10 --call_conf=10 --emit_mode gvcf ${sample}_GVCF.vcf.gz
# sentieon driver --interval $INTERVAL -r $fasta -t $nt -i deduped.bam -q recal_data.table --algo Haplotyper -d $dbsnp --emit_conf=10 --call_conf=10 ${sample}.vcf.gz
sentieon driver --interval $INTERVAL -r $fasta -t $nt -i deduped.bam -q recal_data.table --algo Haplotyper -d $dbsnp --emit_conf=10 --call_conf=10 ../${sample}.vcf

# Matching GATK 4.1
#sentieon driver --interval $INTERVAL -r $fasta -t $nt -i deduped.bam -q recal_data.table --algo Haplotyper -d $dbsnp --genotype_model multinomial --emit_conf 10 --call_conf 10 --emit_mode gvcf ${sample}_GVCF.vcf.gz
