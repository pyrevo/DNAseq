...
##      /path_to_normal/FR1_S16_L001_R1_001.fastq.gz --> "_L001_R1_001.fastq.gz"
##      /path_to_normal/FR1_S16_L001_R2_001.fastq.gz --> "_L001_R2_001.fastq.gz"
################################################################################
...
rule markdup:
    ...
    shell:
        ...
        """
        sentieon driver -t {threads} -i {input.bam1} --algo LocusCollector --fun score_info {output.ns} >> {log} 2>&1
        sentieon driver -t {threads} -i {input.bam1} --algo Dedup --rmdup --score_info {output.ns} --metrics {output.dm} {output.bam} >> {log} 2>&1
        """
...
rule variant_calling:
    ...
    shell:
        """
        sentieon driver --interval {input.bed} -r {input.ref} -t {threads} -i {input.bam} -q {input.rdt} --algo Haplotyper --dbsnp {dbsnp} --emit_mode={params.mode} --emit_conf={params.emit} --call_conf={params.call} {output.vcf} >> {log} 2>&1
        """
